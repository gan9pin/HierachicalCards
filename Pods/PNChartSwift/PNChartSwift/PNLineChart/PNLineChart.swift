...
public class PNLineChart: UIView{
        ...
        didSet{
            ...
            if showLabel {
                ...
                for var index = 0;index < xLabels.count; ++index {
                    ...
                    if(xLabelStep > 1){
                        labelX = labelX - xLabelWidth * (xLabelStep - 1) / 2
                    }
                    let label:PNChartLabel = PNChartLabel(frame: CGRect(x:  labelX, y: chartMargin + chartCavanHeight!,
                        width: xLabelWidth * xLabelStep, height: chartMargin))
                    label.textAlignment = NSTextAlignment.Center
                    label.text = labelText as String
                    if(index % Int(xLabelStep) == 0){
                        addSubview(label)
                    }
                }
            }else {
    ...
    // my edit NSArray型である必要性がなかったので、Int型でｙ軸を分割する数にへんこうした
    public var yLabelsCount: Int = 0{
        didSet{
            
            yLabelNum = CGFloat(yLabelsCount)
            ...
            for(var i = 0 ; i < yLabelsCount + 1 ; i++)
            {
                ...
                label.text = NSString(format:yLabelFormat, yValueMin + (yStep * index)) as String
                ++index
                addSubview(label)
            }
        }
...
//                yLabels = yLabelsArray as NSArray
    ...
    public var xLabelStep:CGFloat = 1
